...
#Depending on the result, the function should return the strings x is prime (for prime numbers) or x is not a prime number (a * b = x), 
...
#Note: Starting with this exercise, we will provide public tests that fail by default. The tests we provide will pass for a correct solution, 
#but keep in mind that the grading system runs many more exhaustive tests, so if the public test passes, 
#that does not necessarily mean that your solution is 100% correct, as it might fail on certain edge or corner cases.
