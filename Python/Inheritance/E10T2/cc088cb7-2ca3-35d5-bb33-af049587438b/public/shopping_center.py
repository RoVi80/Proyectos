...
class ShoppingCenter:
    ...
    def __init__(self, capital, shops:list):
        if shops == []:
            raise Warning("There are no shops in Shopping Center")
    ...
    def grant_loan(self, shop, interest, amount):
        if shop not in self.__shops: raise Warning("Shop not in shopping center")
        if amount > self._capital: raise Warning("Not enough k")
